...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // who owns each shared URN + do they have access (would the guard refuse or release?)
    const shared = rows(await db.execute(sql`
      SELECT la.linkedin_person_urn AS urn, u.email, u.subscription_status AS sub, u.subscription_plan AS plan,
      ...
      WHERE la.page_type=personal AND la.linkedin_person_urn IN (
        SELECT linkedin_person_urn FROM linkedin_accounts
        WHERE linkedin_person_urn IS NOT NULL AND page_type=personal
        GROUP BY 1 HAVING count(DISTINCT user_id) > 1)
      ORDER BY la.linkedin_person_urn, la.created_at`))
    return NextResponse.json({ elizabeth_accounts: accounts, urns_shared_across_users: dupes, shared_detail: shared })
  } catch (e) {
