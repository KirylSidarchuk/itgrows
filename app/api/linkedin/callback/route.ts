...
export async function GET(req: NextRequest) {
  ...
  const state = searchParams.get("state") // userId or userId:type
  ...
  // Parse userId and optional flow type from state (format: "userId" or "userId:personal" or "userId:company")
  ...
  try {
    ...
    // For company-only flow, skip personal profile fetches (openid scopes not available)
    if (connectType === "personal") {
    ...
    if (connectType === "personal") {
      ...
      try {
        ...
        const hasRealProfileData = !!(effectiveHeadline || scrapedPosition || scrapedCompany)
        if (existingBrief.length === 0 && hasRealProfileData) {
          ...
          const profileSummary = [
            ...
            scrapedPosition ? `Current job title: ${scrapedPosition}` : null,
            scrapedCompany ? `Current company: ${scrapedCompany}` : null,
          ...
          if (llmRes.ok) {
            ...
            if (jsonMatch) {
              ...
              await db.insert(linkedinBriefs).values({
                ...
                tone,
                companyName: inferred.company_name || scrapedCompany || null,
